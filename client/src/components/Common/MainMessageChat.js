    ...
    if (loading === false) {
      if (lastChatId !== props.chat._id) {
        ...
        if (props.chat.messages.length > 0 && messages.length > 0) {
          if (
            props.chat.messages[0].id !== messages[0]._id &&
            props.chat.messages.length !== messages.length
          ) {
            getMessages();
            setLoading(true);
            return;
    ...
    return (
      <div className="col main-message-chat absolute-center-pin full-height">
        <ChatHeader
          ...
          unselectChat={props.unselectChat}
        ...
        <form
          className="padding-16 chat-input-field-position"
        ...
        >
          ...
          { <div onClick={() => setIsEmojiPickerVisibile(!isEmojiPickerVisibile)}>
            emoji
          </div> }
        </form>
      </div>
