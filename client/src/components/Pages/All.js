...
const All = (props) => {
  ...
  const loadChats = () => {
    if (document.getElementById("username").value) {
      ...
      axios
        .get(
          "/api/chats/searching/" + document.getElementById("username").value
        )
        .then((response) => {
  ...
  const applyActiveUserItem = (id) => {
    document
      .getElementById("chat-item-" + id)
      .classList.add("user-list-item-active");
  };
  const removeActiveUserItem = (id) => {
    document
      .getElementById("chat-item-" + id)
      .classList.remove("user-list-item-active");
  };
  ...
  useEffect(() => {
    ...
    if (
      selectedChat === null &&
      auth.loadFromRedirect !== null &&
      auth.loadFromRedirect !== undefined &&
      chats !== null
    ) {
      ...
      <div className="row">
        <div className="side-col white-bg full-height">
            ...
            <div className="col padding-24 shadow">
              ...
              <div className="row">
                <div className="search-field">
                  <img
                    src={process.env.PUBLIC_URL + "/icons/search-solid.svg"}
                    alt=""
                  />
                  <input
                    onChange={loadChats}
          ...
          <div className="row scrollable">
            <ChatList
              retrieveChatId={retrieveChatId}
              searching={searching}
              chats={chats}
            />
          </div>
        </div>
        <div className="col blue-bg hide-on-mobile">
